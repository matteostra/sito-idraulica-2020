<article <?php post_class( 'section-inner' ); ?> id="post-<?php the_ID(); ?>">
			...
			// */
			 ...
			 if ( is_page(128) ) {?>
				 ...
				 <?php while ($query -> have_posts() ) : $query -> the_post(); $counter++; ?>
					 ...
					 <?php
					 		$url_slide = wp_get_attachment_image_src( get_post_thumbnail_id(), $size = 'chaplin_fullscreen');
							$url_slide = $url_slide[0];
					  ?>

					 <div class="mySlides fade">
						<img id="img-slide" src="<?php echo $url_slide; ?>" style="width: 100%;">
						<div class="text"> <h2> <?php the_title(); ?></h2> <p> <?php the_content();?> </div>
					</div>
 ...
 function showSlides(n) {
   ...
   $(document).ready(function(){
		 $(".cover-header").addClass('none');
	 });

 </script>
				<?php
			 	} else {?>
 			 	<h1 style="margin: 50px 0 0 0" >loooo</h1>
 				<?php
			 	}
